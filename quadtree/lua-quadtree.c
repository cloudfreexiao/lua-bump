...
static int quad_insert(lua_State *L)
{
...
static int quad_remove(lua_State *L)
{
    struct Quadtree *qt = luaL_checkudata(L, 1, MT_NAME);
...
static int quad_cleanup(lua_State *L)
{
    struct Quadtree *qt = luaL_checkudata(L, 1, MT_NAME);
    qt_cleanup(qt);
    return 0;
...
static int quad_query(lua_State *L)
{
    struct Quadtree *qt = luaL_checkudata(L, 1, MT_NAME);
...
LUA_LIB_API int luaopen_quadtree(lua_State *L)
{
    out_put = il_create(1);
    luaL_checkversion(L);
    luaL_Reg l[] = {
