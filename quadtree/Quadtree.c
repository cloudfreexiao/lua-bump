...
void leaf_insert(Quadtree* qt, int node, int depth, int mx, int my, int sx,
                 int sy, int element)
{
...
void qt_create(Quadtree* qt, int width, int height, int max_elements,
               int max_depth)
{
              ...
              int qbtm, int omit_element)
{
    ...
    for (j = 0; j < il_size(leaves); ++j) {
        ...
        while (elt_node_index != -1) {
            ...
            const int rgt = il_get(qt->elts, element, elt_idx_rgt);
            const int btm = il_get(qt->elts, element, elt_idx_btm);
            if (!qt->temp[element] && element != omit_element &&
